...
class _ControlScreenState extends State<ControlScreen> {
  ...
  // Colors
  //static const Color _bg = Color(0xFF1E2B4B);
  ...
  void findTargetCharacteristic() {
    for (BluetoothService service in services) {
      for (BluetoothCharacteristic c in service.characteristics) {
        if (c.uuid.toString().contains('ff00')) selectedChar = c; //read
        if (c.uuid.toString().contains('ff01')) minChar = c; //write
        if (c.uuid.toString().contains('ff02')) maxChar = c; //write
        if (c.uuid.toString().contains('ff03')) targetChar = c; //write
      }
    }
  }
                      ...
                      onSubmitted: (value) async {
                        ...
                        if (displayTarget.isNotEmpty &&
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text("Min can't be greater than Target"),
                            ),
                          );
